...
const SQLSERVER_BASE_URL = IS_LOCAL
    ? 'http://localhost:3001'   // 
    : 'https://proyecto-cloudflare.apiprueba2025.workers.dev';
...
// ============================================================
const SERVICIOS = {
    firebase:   { nombre: 'Firebase',       icono: 'fa-database' },
    cloudflare: { nombre: 'Cloudflare',     icono: 'fa-cloud' },
    sheets:     { nombre: 'Google Sheets',  icono: 'fa-table' },
    sqlserver:  { nombre: 'SQL Server',     icono: 'fa-server' }
};
...
function updateServiceUI() {
    ...
    if (servicio) {
        indicator.innerHTML = `<i class="fas ${servicio.icono}"></i> ${servicio.nombre}`;
    }
...
async function cargarSeriesCloudflare() {
    try {
        ...
        updateServiceUI();
        showError("Error al conectar con Cloudflare. Cambiando a SQL Server...");
        await cargarSeriesSQLServer();
    }
}
...
async function cargarSeriesSQLServer() {
    try {
        ...
        showSuccess(`✅ Cargadas ${datosTemporales.length} portadas desde SQL Server (Azure SQL)`);
    } catch (error) {
        console.error("Error cargando desde SQL Server:", error);
        servicioActivo = 'sheets';
        updateServiceUI();
        showError("Error al conectar con SQL Server. Cambiando a Google Sheets...");
        await cargarSeriesGoogleSheets();
    }
}
