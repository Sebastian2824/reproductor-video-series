...
const SQLSERVER_BASE_URL = IS_LOCAL
    ? 'http://localhost:3001'   // ⚠️ Ajusta el puerto si tu nuevo backend usa otro
...
function updateServiceUI() {
    ...
    // Actualizar indicadores de recomendaciones
...
async function cargarEpisodiosDesdeSQLServer() {
    try {
        ...
        if (!Array.isArray(servidores) || servidores.length === 0) {
            // Intentar obtener idiomas alternativos
...
async function cargarEpisodiosDesdeGoogleSheets() {
    try {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            if (idiomaCSV !== idioma) continue;
            if (!episodioCSV || !iframeCSV || !servidorCSV) continue;
...
function mostrarRecomendaciones(temporadasMismaSerie = []) {
    ...
    if (recomendacionesGridMobile) recomendacionesGridMobile.innerHTML = loadingHTML;
    ...
    if (recomendaciones.length === 0) {
        ...
        if (recomendacionesGridMobile) recomendacionesGridMobile.innerHTML = noResultsHTML;
        return;
    }

    const cardsHTML = generarCardsHTML(recomendaciones);
    recomendacionesGrid.innerHTML = cardsHTML;
    if (recomendacionesGridMobile) recomendacionesGridMobile.innerHTML = cardsHTML;
}
...
function seleccionarRecomendaciones(temporadasMismaSerie = []) {
    ...
    // Buscar series similares por palabras comunes
    ...
    for (const aleatoria of seriesAleatorias) {
        ...
        if (!esDuplicado) resultado.push(aleatoria);
    }
...
async function cargarEpisodiosDeSerieRecomendada() {
    ...
    if (servicioActivo === 'firebase') {
        ...
        await cargarEpisodiosDesdeSQLServerParaRecomendada();
    } else {
...
async function cargarEpisodiosDesdeSQLServerParaRecomendada() {
    try {
        console.log(`🔌 Cargando serie recomendada desde SQL Server...`);

        // Reutilizamos la misma función con manejo de errores específico
        const servidoresResponse = await fetch(
            `${SQLSERVER_BASE_URL}/servidores?serie=${encodeURIComponent(nombreSerie)}&temporada=${encodeURIComponent(temporada)}&idioma=${encodeURIComponent(idioma)}`
        );
...
async function cargarEpisodiosDesdeGoogleSheetsParaRecomendada() {
    try {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            if (nombreSerieCSV !== nombreSerie || temporadaCSV !== temporada) continue;
            if (i === 1 && idiomaCSV !== idioma) idioma = idiomaCSV;
            if (idiomaCSV !== idioma) continue;
            if (!episodioCSV || !iframeCSV || !servidorCSV) continue;
        ...
        if (episodios.length === 0) {
            throw new Error(`No se encontraron episodios en Google Sheets`);
        }
...
async function cargarEpisodiosDesdeCloudflare() {
    try {
        ...
        if (servicioActivo === 'cloudflare') {
            servicioActivo = 'sqlserver';   // 👈 ahora va a SQL Server en vez de Firebase
            updateServiceUI();
            await cargarEpisodiosDesdeSQLServer();
        }
    }
...
async function cargarEpisodiosDesdeCloudflareParaRecomendada() {
    try {
        ...
        if (!servidores || servidores.error) {
            throw new Error("Error obteniendo servidores de Cloudflare");
        }
