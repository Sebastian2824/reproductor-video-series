  ...
  async function cargarEpisodios() {
    ...
    try {
      ...
      actualizarIndicador();
      await cargarEpisodios(); // reintentar
    }
  }
  ...
  function changeVideo(plataforma) {
    ...
    if (!embed) {
      // Buscar el primer episodio disponible en este servidor
      const idx = episodios.findIndex(ep => ep.embeds[plataforma]);
      if (idx !== -1) {
        currentEpisode = idx;
        document.getElementById('episodeSelector').value = currentEpisode;
        // Reintentar con el nuevo episodio
        changeVideo(plataforma);
        return;
      }
      // Si no hay ningún episodio, mostrar mensaje con opciones
      container.innerHTML = `
