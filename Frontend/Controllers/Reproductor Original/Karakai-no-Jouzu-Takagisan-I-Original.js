...
// Orden de fallback (rotación circular)
const ORDEN_SERVICIOS = ['firebase', 'cloudflare', 'sqlserver', 'sheets'];
...
async function cargarEpisodios() {
    ...
    try {
        ...
        const idx = ORDEN_SERVICIOS.indexOf(servicioActivo);
        const next = ORDEN_SERVICIOS[(idx + 1) % ORDEN_SERVICIOS.length];
        servicioActivo = next;
...
function actualizarIndicador() {
    ...
    const servicio = SERVICIOS[servicioActivo] || SERVICIOS.firebase;
    label.innerHTML = `<i class="fas ${servicio.icono}"></i> Servicio: ${servicio.nombre}`;
}
